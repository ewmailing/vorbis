...
#
#  Note: There are now two flavors of this toolchain file: 32-bit toolchain and 64-bit toolchain.
#
#  Usage Linux:
...
#
#   $ SET ANDROID_NDK_ROOT=C:\absolute\path\to\the\android-ndk
#   $ mkdir build && cd build
#   $ cmake.exe -G"MinGW Makefiles"
#       -DCMAKE_TOOLCHAIN_FILE=path\to\the\android.toolchain.cmake
#       -DCMAKE_MAKE_PROGRAM="%ANDROID_NDK_ROOT%\prebuilt\windows\bin\make.exe" ..
#   $ "%ANDROID_NDK_ROOT%\prebuilt\windows\bin\make.exe"
#
#
#  Options (can be set as cmake parameters: -D<option_name>=<value>):
#    ANDROID_NDK=/opt/android-ndk - path to the NDK root.
#      Can be set as environment variable ANDROID_NDK_ROOT. Can be set only at first cmake run.
#
#    ANDROID_STANDALONE_TOOLCHAIN=/opt/android-toolchain - path to the
#      standalone toolchain. This option overrides ANDROID_NDK 
#      (and subsequentally the enviroment variable ANDROID_NDK_ROOT).
...
#           See ${ANDROID_NDK_ROOT}/docs/CPU-ARCH-ABIS.html for the documentation.
...
#      (ANDROID_NDK - environment variable as legacy fallback)
...
#     [+] updated for NDK r9 (standalone)
#     [+] Addded Perl helper scripts to make it easier to build multiple architectures in one shot.
...
#     [~] Changed armeabi-v7a to use -mfpu=vfpv3-d16 instead of -mfpu=vfp to conform with stock Android.
#     [~] Changed VFPV3 mode to use -mfpu=vfpv3-d32
#     [~] Use --sysroot=${ANDROID_SYSROOT} to match stock Android
...
#     [~] Added -funswitch-loops -finline-limit=300 to match stock Android
#     [~] Changed arm debug optimization level to O0 from Os
#     [~] Separated ANDROID_CXX_FLAGS into ANDROID_CXX_FLAGS and ANDROID_C_FLAGS
...
# There was an Android NDK bug with armv7 and vfpv3 in r7b which caused crashes according to
# http://stackoverflow.com/questions/11831648/android-ndk-arm-build-settings-to-run-on-most-devices
# Not supporting r7b or below seems wiser. Supporting ancient Android NDKs is rather pointless since part of the reason for the updates is to fix bugs.
#set( ANDROID_SUPPORTED_NDK_VERSIONS ${ANDROID_EXTRA_NDK_VERSIONS} -r8d -r8b -r8 -r7c -r7b -r7 -r6b -r6 -r5c -r5b -r5 "" )
set( ANDROID_SUPPORTED_NDK_VERSIONS ${ANDROID_EXTRA_NDK_VERSIONS} -r9 -r8d -r8b -r8 -r7c "" )
...
if(ANDROID_SYSROOT MATCHES "[ ;\"]")
 set( ANDROID_C_FLAGS "--sysroot=\"${ANDROID_SYSROOT}\"" )
...
if( ARMEABI OR ARMEABI_V7A )
 ...
 set( _CMAKE_CXX_FLAGS "-fpic -fstack-protector -no-canonical-prefixes -Wa,--noexecstack" )
 set( _CMAKE_C_FLAGS "-fpic -fstack-protector -no-canonical-prefixes -Wa,--noexecstack" )
 ...
 # set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -fsigned-char" )
 # set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -fsigned-char" )
elseif( X86 )
 set( _CMAKE_CXX_FLAGS "-no-canonical-prefixes -funwind-tables -fstack-protector -Wa,--noexecstack" )
 set( _CMAKE_C_FLAGS "-no-canonical-prefixes -funwind-tables -fstack-protector -Wa,--noexecstack" )
elseif( MIPS )
...
# C89 mode is annoying for most people
set( _CMAKE_C_FLAGS "-std=gnu99" )

if( ANDROID_USE_STLPORT )
 set( _CMAKE_CXX_FLAGS "${_CMAKE_CXX_FLAGS} -fno-rtti -fno-exceptions" )
 # set( _CMAKE_C_FLAGS "${_CMAKE_C_FLAGS}" )
else()
...
if( ARMEABI OR ARMEABI_V7A )
 if( NOT ANDROID_FORCE_ARM_BUILD AND NOT ARMEABI_V6 )
  ...
  # EW: Note that Android's normal build process uses 
  # -mthumb -Os -g -DNDEBUG -fomit-frame-pointer -fno-strict-aliasing -finline-limit=64
  # and for debug, appends: -O0 -UNDEBUG -marm -fno-omit-frame-pointer 
  set( _CMAKE_CXX_FLAGS_RELEASE "-mthumb -O3 -finline-limit=64" )
  set( _CMAKE_C_FLAGS_RELEASE   "-mthumb -O3 -finline-limit=64" )
  set( _CMAKE_CXX_FLAGS_DEBUG "-marm -O0 -g -finline-limit=64" )
  set( _CMAKE_C_FLAGS_DEBUG   "-marm -O0 -g -finline-limit=64" )
 else()
...
elseif( X86 )
 ...
 set( _CMAKE_CXX_FLAGS_DEBUG "-O0 -g -funswitch-loops -finline-limit=300" )
 set( _CMAKE_C_FLAGS_DEBUG   "-O0 -g -funswitch-loops -finline-limit=300" )
elseif( MIPS )
 set( _CMAKE_CXX_FLAGS_RELEASE "-O3 -funswitch-loops -finline-limit=300" )
 set( _CMAKE_C_FLAGS_RELEASE   "-O3 -funswitch-loops -finline-limit=300" )
...
# ABI-specific flags
if( ARMEABI_V7A )
 set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -march=armv7-a -mfloat-abi=softfp" )
 set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -march=armv7-a -mfloat-abi=softfp" )
 if( NEON )
  set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -mfpu=neon" )
  set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -mfpu=neon" )
 elseif( VFPV3 )
  set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -mfpu=vfpv3-d32" )
  set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -mfpu=vfpv3-d32" )
 else()
  # Default Android armv7 builds with -mfpu=vfpv3-d16.
  # According to this:
  # http://stackoverflow.com/questions/11831648/android-ndk-arm-build-settings-to-run-on-most-devices
  # there was a bug in r7b which caused crashes.
  # But r9 is the current as of this writing so developers really need to upgrade their ndk.
  set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -mfpu=vfpv3-d16" )
  set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -mfpu=vfpv3-d16" )
 endif()
elseif( ARMEABI_V6 )
 set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -march=armv6 -mfloat-abi=softfp -mfpu=vfp" )
 set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -march=armv6 -mfloat-abi=softfp -mfpu=vfp" )
elseif( ARMEABI )
 set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS} -march=armv5te -mtune=xscale -msoft-float" )
 set( ANDROID_CXX_FLAGS "${ANDROID_CXX_FLAGS} -march=armv5te -mtune=xscale -msoft-float" )
elseif( X86 )
 set( ANDROID_C_FLAGS "${ANDROID_C_FLAGS}" )#sse?
...
# Android by default passes --function-sections for C and C++ but does not pass -fdata-sections.
# Android also does not pass -Wl,--gc-sections
...
if( ARMEABI_V7A )
 ...
 # Android passes all this stuff by default.
...
# finish flags
set( ANDROID_C_FLAGS    "${ANDROID_C_FLAGS}"    CACHE INTERNAL "Extra Android C compiler flags")
set( ANDROID_CXX_FLAGS    "${ANDROID_CXX_FLAGS}"    CACHE INTERNAL "Extra Android C++ compiler flags")
...
# EW: Using BOTH for library and include so the user can specify -DCMAKE_LIBRARY_PATH for 3rd party prebuilt NDK modules.
...
#   ANDROID_C_FLAGS : C compiler flags required by Android platform
#   ANDROID_CXX_FLAGS : C++ compiler flags required by Android platform
